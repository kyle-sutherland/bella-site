...
function renderRichTextNode(node: any, index?: number): React.ReactNode {
  ...
  if (node.type === 'paragraph') {
    const children = node.children?.map((child: any, i: number) => renderRichTextNode(child, i)) || [];
  ...
  if (node.type === 'list') {
    const items = node.children?.map((child: any, i: number) => renderRichTextNode(child, i)) || [];
...
export function postRenderer(section: any, index: number) {
  ...
  if (section.type) {
    return renderRichTextNode(section, index);
  }
